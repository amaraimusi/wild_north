...
use App\Models\BackImg;
use Illuminate¥Support¥Facades¥DB;

class BackImgController extends AppController
{
	...
	public function index(){
		...
		return view('back_img.index', compact('data', 'crudBaseData', 'crud_base_json'));
	...
	public function ajax_reg(){
		...
		if(!empty($_FILES)){
			// CBBXS-20271
			$img_fn = $this->cb->makeFilePath($_FILES, "storage/BackImg/y%Y/{$ent['id']}/%unique/orig/%fn", $ent, 'img_fn');
	...
	private function init(){
		...
		$kensakuJoken=[
				...
				// CBBXS-2000
				['name'=>'kj_id', 'def'=>null],
				['name'=>'kj_back_img_name', 'def'=>null],
		...
		$fieldData = ['def'=>[
			...
			'id'=>[
					'name'=>'ID',//HTMLテーブルの列名
					'row_order'=>'BackImg.id',//SQLでの並び替えコード
					'clm_show'=>1,//デフォルト列表示 0:非表示 1:表示
			],
			'back_img_name'=>[
					'name'=>'背景画像名',
					'row_order'=>'BackImg.back_img_name',
					'clm_show'=>1,
			],
			'img_fn'=>[
					'name'=>'画像ファイル名',
					'row_order'=>'BackImg.img_fn',
					'clm_show'=>1,
			],
			'sort_no'=>[
					'name'=>'順番',
					'row_order'=>'BackImg.sort_no',
					'clm_show'=>0,
			],
			'delete_flg'=>[
					'name'=>'無効フラグ',
					'row_order'=>'BackImg.delete_flg',
					'clm_show'=>0,
			],
			'update_user'=>[
					'name'=>'更新者',
					'row_order'=>'BackImg.update_user',
					'clm_show'=>0,
			],
			'ip_addr'=>[
					'name'=>'IPアドレス',
					'row_order'=>'BackImg.ip_addr',
					'clm_show'=>0,
			],
			'created'=>[
					'name'=>'生成日時',
					'row_order'=>'BackImg.created',
					'clm_show'=>0,
			],
			'modified'=>[
					'name'=>'更新日',
					'row_order'=>'BackImg.modified',
					'clm_show'=>0,
			],
		...
		$model = new BackImg(); // モデルクラス
		
		$crudBaseData = [
				'fw_type' => 'laravel7',
				'model_name_c' => 'BackImg',
				'tbl_name' => 'back_imgs', // テーブル名をセット
	...
	private function getDataForDownload(){
		...
		//セッションから検索条件情報を取得
		$kjs=session('back_img_kjs');

		// セッションからページネーション情報を取得
		$pages = session('back_img_pages');
	...
	public function bulk_reg(){
		...
		$res = $bulkReg->reg('back_imgs', $param);
