 ...
 */
	public static function stripAll($str) {
		return SanitizeCustom::stripScripts(
	...
	public static function clean($data, $options = array()) {
		...
		if (is_array($data)) {
			foreach ($data as $key => $val) {
				$data[$key] = SanitizeCustom::clean($val, $options);
			}
			return $data;
		}
		...
		if ($options['encode']) {
			$data = SanitizeCustom::html($data, array('remove' => $options['remove_html']));
		}
		if ($options['dollar']) {
		...
		if ($options['escape']) {
			$data = SanitizeCustom::escape($data, $options['connection']);
		}
