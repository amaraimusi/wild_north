...
jQuery(()=> {
	...
	$('#back_img_tbl').show();// 高速表示のためテーブルは最後に表示する
...
//			{'field':'back_img_group', 'inp_type':'select', 'list':back_imgGroupList, 'def':2}, 
//			{'field':'back_img_date', 'inp_type':'date', 'def':today}, 
//			{'field':'note', 'inp_type':'text', 'def':'TEST'}, 
//			{'field':'sort_no', 'inp_type':'sort_no', 'def':1}, 
		],
		{
			ajax_url:'back_img/bulk_reg',
			csrf_token:csrf_token,
			ta_placeholder:"Excelからコピーした背景画像名、背景画像数値を貼り付けてください。（タブ区切りテキスト）\n(例)\n背景画像名A\t100\n背景画像名B\t101\n",
		}
	);
...
function calendarViewKShow(){
	// カレンダービューを生成 
	crudBase.calendarViewCreate('back_img_date');
}
